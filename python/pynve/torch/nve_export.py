...
def _export_metadata(model: torch.nn.Module, save_dir: str, *,
    ...
    for name, module in model.named_modules():
        ...
        if module.cache_type == CacheType.Hierarchical:
            ri = getattr(module, 'remote_interface', None)
            if ri is not None and hasattr(ri, 'export_config'):
                entry["remote_ps_config"] = ri.export_config()
            else:
                raise ValueError(
                    f"Hierarchical layer '{name}' has no exportable remote_interface. "
                    "The remote_interface must implement an export_config() method "
                    "that returns a JSON-serializable dict with a 'remote_ps_type' key.")

            if ps_data_paths and name in ps_data_paths:
                keys_path, values_path = ps_data_paths[name]
                entry["remote_ps_data"] = {
                    "keys": keys_path,
                    "values": values_path,
                }
...
def _export_weights(model: torch.nn.Module, save_dir: str):
    """Save LinearUVM / NoCache embedding weights, one .nve file per module.
    ...
    for name, module in model.named_modules():
        if not isinstance(module, NVEmbeddingBase):
            continue
        if module.cache_type not in (CacheType.LinearUVM, CacheType.NoCache):
            continue
...
def save_nve(model: torch.nn.Module, save_dir: str, *,
    ...
    _export_metadata(model, save_dir, ps_data_paths=ps_data_paths)
...
def load_nve_layers(save_dir: str, *, gpu_cache_size_override: int = None):
    ...
    For Hierarchical layers, the remote parameter server is reconstructed
    directly via :class:`pynve.nve.ParameterServerTable` using the
    ``plugin_name`` / ``factory_config`` / ``table_config`` that were
    serialized at export time. If ``remote_ps_data`` paths are present, data
    is reloaded via ``insert_keys_from_filepath``.
    ...
    for entry in metadata:
        ...
        if cache_type == CacheType.Hierarchical:
            ...
            if ps_config.get("remote_ps_type") != "plugin":
                raise ValueError(
                    f"Hierarchical layer '{module_path}' has unsupported "
                    f"remote_ps_type={ps_config.get('remote_ps_type')!r}; "
                    "only 'plugin' is supported.")

            # Recreate the PS directly via the plugin-based ParameterServerTable
            # ctor. The plugin loader (load_host_table_plugin +
            # create_host_table_factory) is itself the registry — no extra
            # indirection needed.
            ps_data_type_str = ps_config["data_type"].lower()
            if "float32" in ps_data_type_str:
                ps_layer_dtype = _nve_mod.DataType_t.Float32
            elif "float16" in ps_data_type_str:
                ps_layer_dtype = _nve_mod.DataType_t.Float16
            else:
                raise ValueError(f"Unsupported PS data_type: {ps_config['data_type']}")

            remote_ps = _nve_mod.ParameterServerTable(
                row_elements=ps_config["row_elements"],
                data_type=ps_layer_dtype,
            ...
            # Optionally reload data from files
            ps_data = entry.get("remote_ps_data")
            if ps_data:
        ...
        elif cache_type in (CacheType.LinearUVM, CacheType.NoCache):
            # Recreate full NVEmbedding / NVEmbeddingBag via __init__
            # (handles memblock, register_for_torch, weight allocation)
            kwargs = dict(optimize_for_training=optimize_for_training, id=layer_id)
            if cache_type == CacheType.LinearUVM:
                kwargs["gpu_cache_size"] = gpu_cache_size
            if is_bag:
                layer = NVEmbeddingBag(
                    num_emb, emb_size, data_type, cache_type, mode, **kwargs)
            else:
                layer = NVEmbedding(
                    num_emb, emb_size, data_type, cache_type, **kwargs)
